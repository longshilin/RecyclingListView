...
public class RecyclingListView : MonoBehaviour
{
    ...
    private RecyclingListScrollType _scrollType = RecyclingListScrollType.Hor;
    ...
    public int RowCount
    {
        ...
        set
        {
            if (rowCount != value)
            {
                ...
                // 更新宽度或高度
                if (_scrollType == RecyclingListScrollType.Hor)
                {
                    UpdateContentWidth();
                }
                else
    ...
    protected virtual void Awake()
    {
        ...
        _scrollType = scrollRect.horizontal ? RecyclingListScrollType.Hor : RecyclingListScrollType.Ver;
    ...
    public float GetRowScrollPosition(int row, ScrollPosType posType)
    {
        ...
        // 列表内容总高度
        float contentHeight = _scrollType == RecyclingListScrollType.Hor ? scrollRect.content.sizeDelta.x : scrollRect.content.sizeDelta.y;
        ...
        return _scrollType == RecyclingListScrollType.Hor ? 1 - inverseLerp : inverseLerp;
    }
    ...
    protected virtual void ReorganiseContent(bool clearContents)
    {
        ...
        float ymin = _scrollType == RecyclingListScrollType.Hor ? -scrollRect.content.localPosition.x : scrollRect.content.localPosition.y;

        // 第一个可见item的索引
        int firstVisibleIndex = (int)(ymin / RowWidthOrHeight());
    ...
    {
        var rect = ChildObj.RectTransform.rect;
        return _scrollType == RecyclingListScrollType.Hor ? rect.width : rect.height;
    }
    ...
    {
        return RowPadding + ChildWidthOrHeight();
    }

    /// <summary>
    /// 获取列表视口的宽度或高度
    /// </summary>
    private float ViewportWidthOrHeight()
    {
    ...
    protected virtual void UpdateChild(RecyclingListViewItem child, int rowIdx)
    {
        ...
        else
        {
            ...
            float offset = RowWidthOrHeight() * (child is CardRecyclingListViewItem ? rowIdx + 1 : rowIdx);
            if (_scrollType == RecyclingListScrollType.Hor)
            {
    ...
    {
        // 列表高度
        float width = ChildObj.RectTransform.rect.width * rowCount + (rowCount - 1) * RowPadding;
        // 更新content的宽度
